...
void flow_conv_int_trunc ( const int N, const int node_trunc, const double lam_o, const double c[], const double s[], const double ds[], double int_term[] );
...
/*
  ...
  Discussion:
    ...
    the MATLAB user can invoke the function by typing:
      ...
      cx = flow_conv_int_trunc ( N, node_thresh, lam_o, C, s, dS )

    The routine computes the flowfield truncating the weighting function
    node_thresh number of nodes upstream
...
*/
  ...
  node_trunc  = mxGetScalar ( prhs[1] );
  lam_o  = mxGetScalar ( prhs[2] ); 
  c      = mxGetPr     ( prhs[3] );
  s      = mxGetPr     ( prhs[4] );
  ds     = mxGetPr     ( prhs[5] );
/*
...
*/
  ...
  flow_conv_int_trunc ( N, node_trunc, lam_o, c, s, ds, int_term );
...
/*
  ...
  Modified:
    ...
    April 27 2015 original source code
    May 29 2021 additional commenting
 ...
 */
{
  ...
  double ig;
  double ig_old;
  trunc = node_trunc-1;  //set to 4-1 for testing
...
for ( i = 1; i <trunc-1; i++ )
  {
      ...
      for ( j = 1; j <= i; j++ )
      {
//           mexPrintf("i,j are %d,%d \n",i,j);
          ig_old = ig;
          ig = c[j] * exp( lam_o * ( s[i]-s[j] ) );
          int_term[i] = int_term[i] + ds[j-1] * ( ig_old + ig ) / 2;
      }
  }
/* loop 2 the truncated part */
for ( i = trunc-1; i <N; i++ )
  {
      ig = c[i-trunc] * exp( lam_o * ( s[i]-s[i-trunc] )); 
      for ( j = i-trunc+1; j <= i; j++ )
      {
//           mexPrintf("loop 2:i,j are %d,%d \n",i,j);
          ig_old = ig;
          ig = c[j] * exp( lam_o * ( s[i]-s[j] ) );
          int_term[i] = int_term[i] + ds[j-1] * ( ig_old + ig ) / 2;
      }
